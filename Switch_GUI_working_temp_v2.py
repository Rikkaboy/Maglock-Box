...
class Application(Frame):
    def __init__(self, master):
		...
		self.fail0a = pygame.mixer.Sound('8b-fail0a.wav')
		self.fail0b = pygame.mixer.Sound('8b-fail0b.wav')
		self.fail0c = pygame.mixer.Sound('8b-fail0c.wav')
		self.fail1 = pygame.mixer.Sound('8b-fail1a.wav')
		self.fail2 = pygame.mixer.Sound('8b-fail2b.wav')
		self.fail3 = pygame.mixer.Sound('8b-fail3c.wav')
		self.fail4a = pygame.mixer.Sound('8b-fail4a.wav')
		self.fail4b = pygame.mixer.Sound('8b-fail4b.wav')
		self.fail4c = pygame.mixer.Sound('8b-fail4c.wav')
    ...
    def CheckProgress(self):
        ...
        if self.pressed == self.code[self.progress]:
            ...
            #failure sounds for how long they've pushed buttons
			if self.buttonSnd == "d"
				self.buttonSnd = "a"
			if self.progress == 0 and self.buttonSnd == "c"
				self.fail0c.play()
				self.buttonSnd = "d"
			if self.progress == 0 and self.buttonSnd == "b"
				self.fail0b.play()
				self.buttonSnd = "c"
			if self.progress == 0 and self.buttonSnd == "a"
				self.fail0a.play()
				self.buttonSnd = "b"
			if self.progress == 1
				self.fail1.play()
			...
			if self.progress == 3
				self.fail3.play()
			if self.progress >= 4 and self.buttonSnd == "c"
				self.fail4c.play()
				self.buttonSnd = "d"
			if self.progress >= 4 and self.buttonSnd == "b"
				self.fail4b.play()
				self.buttonSnd = "c"
			if self.progress >= 4 and self.buttonSnd == "a"
				self.fail4a.play()
				self.buttonSnd = "b"
			
			#self.failSnd.play() 
			#cancel out this failure sound when length ones put in
			time.sleep(4) #was 2 before I put in longer sounds
            self.progress = 0
            self.lbl1['text'] = "Code Entered: "
            for x in range(len(self.code)):
                self.lbl1['text'] += "*"
